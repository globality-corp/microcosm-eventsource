...
class EventStoreAsync(StoreAsync):
    """
    ...
    async def upsert_index_elements(self):
        """
    ...
    async def upsert_on_index_elements(self, instance):
        """
    ...
    def _where(self,
        ...
        if container_id is not None:
            query = query.where(self.model_class.container_id == container_id)
        if event_type is not None:
            query = query.where(self.model_class.event_type == event_type)
        if clock is not None:
            query = query.where(self.model_class.clock == clock)
        if min_clock is not None:
            query = query.where(self.model_class.clock >= min_clock)
        if max_clock is not None:
            query = query.where(self.model_class.clock <= max_clock)
        if parent_id is not None:
            query = query.where(self.model_class.parent_id == parent_id)
        if version is not None:
            query = query.where(self.model_class.version == version)

        return super(EventStoreAsync, self)._where(query, **kwargs)
